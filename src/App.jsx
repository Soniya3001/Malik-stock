...
async function fetchQuote(sym, tdKey) {
  ...
  if (!res.ok) throw new Error("Backend server error: " + res.status + ". Railway server check karo.");
  var d = await res.json();
  if (d.error) throw new Error(d.error);
  if (d.status === "error") throw new Error("Twelve Data: " + (d.message || "Symbol not found"));
  ...
  return (
    <div style={{minHeight:"100vh",background:"#030810",fontFamily:"'Courier New',monospace",color:"#a0bcd0"}}>
      ...
      <div style={{maxWidth:980,margin:"0 auto",padding:"13px 12px"}}>
        ...
        {/* API KEYS */}
        <div style={{background:"#001e10",border:"1px solid #00e67630",borderRadius:9,padding:"11px 13px",marginBottom:10}}>
          <div style={{fontSize:8,color:"#00e676",letterSpacing:2,marginBottom:5}}>🚀 BACKEND SERVER — RAILWAY</div>
          <div style={{fontSize:9,color:"#3a7a4a",lineHeight:1.6}}>
            ✅ Connected: <span style={{color:"#00e676"}}>nk-backend-production-f95a.up.railway.app</span>
          </div>
          <div style={{fontSize:8,color:"#1a4060",marginTop:4}}>
            Saare NSE stocks kaam karenge — RELIANCE, NIFTY, BANKNIFTY sab! 🎉
          </div>
        </div>
